...
CF3_RS_PROFILE_RSS=1 /usr/bin/time -v -o "$TIMEFILE" "$BIN" "${ARGS[@]}" >>"$LOG" 2>&1 &
RUNPID=$!
...
if [[ ! -s $TSV ]]; then
  printf 'variant\timpl\tdataset\tmode\tcolor\tthreads\tk\tmin_len\tstatus\twall\tmax_rss_kb\tfd_peak\tpart_workers\tpartition_s\tbuild_s\tlocal_s\tcontract_s\texpand_s\tunitigs\tbases\twork_bytes\tfasta_bytes\ttag\n' > "$TSV"
fi
printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
  ...
  "${UNITIGS:-}" "${BASES:-}" "${WORKBYTES:-}" "$FASTA" "$TAG" >> "$TSV"

rm -f "$TIMEFILE" "$FDPEAK_FILE"
