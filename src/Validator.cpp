...
bool Validator::validate(const std::string& bbhash_file_name, const uint16_t thread_count)
{
    ...
    bool valid_kmer_set = validate_kmer_set();
...
void Validator::build_unitig_tables()
{
    ...
    while(input >> unitig)
    {
        ...
        if(unitig.length() == k)
        {
            cuttlefish::kmer_t kmer(unitig);
            uint64_t kmer_hash = mph->lookup(kmer.canonical());
        ...
        {
            cuttlefish::kmer_t last_kmer = cuttlefish::kmer_t(unitig.substr(unitig.length() - k, k));
            cuttlefish::kmer_t kmer_fwd(unitig, 0);
            cuttlefish::kmer_t kmer_bwd = last_kmer.reverse_complement();

            uint64_t kmer_fwd_hash = mph->lookup(kmer_fwd.canonical());
            uint64_t kmer_bwd_hash = mph->lookup(kmer_bwd.canonical());
...
bool Validator::walk_sequence(const char* seq, const size_t seq_len)
{
    size_t kmer_idx = 0;
...
{
    cuttlefish::kmer_t kmer(seq, start_idx);
    uint64_t kmer_hash = mph->lookup(kmer.canonical());
