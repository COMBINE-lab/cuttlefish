...
## Exact color fidelity by sorting buckets at read time

The colored class hash (XOR of source hashes with consecutive-dedup) is
exact only when each source's occurrences of a vertex are consecutive in its
bucket. C++ guarantees that by counting-sorting every bucket per source
window at write time; the Rust partitioner's source-ownership scheme only
approximated it, leaving a narrow window -- a repeated k-mer straddling a
chunk boundary with an interposed shared source -- where a class hash could
alias another registered class and inherit its color set on the
verification-free reuse path. Earlier full color-content comparisons at 1k
and 5k genomes passed, consistent with rare rather than absent.

The fix moves the sort to consumption: `ColoredBucketRecords::load_sorted`
materializes a bucket's records once, counting-sorts them by source id, and
feeds both the hashing pass and the wanted-color second pass from the same
memory -- deleting the second pass's disk re-read and re-decode as a side
effect. Write-side ordering is now irrelevant to color correctness, which
also removes the design coupling that constrained colored-partition
parallelization. A regression test pins the property: interleaved and
grouped arrival orders must hash identically, and a {1,2} class must not
alias a {2} class.

## The inflate budget as a hard total-concurrency cap

The decompression budget is now explicit and compliant: readers + sharding
workers + decode slots together never exceed `--threads`. One shared
rapidgzip `DecoderPool` per partition run holds the decode share (half the
post-reader budget in the streamed path; exactly the workers' leftover in
the colored path, where the many-small-files regime stays on the serial
decoder via a 4 MiB compressed-size threshold). Decoders spawn lazily
against the pool and rapidgzip's per-stream calibration trims each file's
use, so an early finisher's capacity flows to laggards. Measured: the pool
itself is free (mbal parity with the per-reader static split at equal
concurrency), and colored 150k is unchanged (budget 0 pool, threshold-serial
decode). The compliant streamed split's own numbers accompany the next
campaign.
